...
var fs = require('fs');
var url = require('url');
//var hdispatcher = require('./httpdispatcher');
...
dispatcher.onGet('/', function (req, res) {
  fs.readFile('./static/index.html', function (err, f) {
    if (err) {
      res.writeHead(404, {'Contne-Type': 'text/plain'});
      res.write('Static file read error. ' + err.toString());
      res.end();
    } else {
...
dispatcher.onGet('/time', function (req, res) {
  res.writeHead(200, {'Content-Type': 'application/json'});
  var date = new Date();
  res.write(JSON.stringify({'time': date.getTime().toString()}));
  res.end();
});

var lastPillTime = new Date(0);

dispatcher.onGet('/pillStatus', function (req, res) {
  res.writeHead(200, {'Content-Type': 'application/json'});
  res.write(JSON.stringify({'time': lastPillTime.getTime().toString()}));
  res.end();
});

dispatcher.onGet('/pillSet', function (req, res) {
  lastPillTime = new Date();
  res.writeHead(200, {'Content-Type': 'application/json'});
  res.write(JSON.stringify({'time': lastPillTime.getTime().toString()}));
  res.end();
});
...
http.createServer(function (req, res) {
  if (url.parse(req.url, true).pathname.substring(0, 8) == '/static/') {
    var pathName = url.parse(req.url, true).pathname.substring(8);
    if (files.indexOf(pathName) != -1) {
      fs.readFile('./static/' + pathName, function (err, f) {
        if (err) {
          res.writeHead(404, {'Contne-Type': 'text/plain'});
          res.write('Static file read error. ' + err.toString());
          res.end();
